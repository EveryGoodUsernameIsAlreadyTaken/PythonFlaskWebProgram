#CREATE SCHEMA IF NOT EXISTS SCHOOLMGMTDB;
USE SCHOOLPYDB;

DELIMITER $$
DROP PROCEDURE IF EXISTS `PQ_NEW_CID`$$
CREATE PROCEDURE `PQ_NEW_CID` ()
BEGIN
...
DROP PROCEDURE IF EXISTS `PQ_UPDATE_STUDENT`$$
CREATE PROCEDURE `PQ_UPDATE_STUDENT` (
	A_SID			INT,
...
BEGIN
    UPDATE STUDENTS SET USERNAME = A_USERNAME, PASSWORD = A_PASSWORD, FIRSTNAME = A_FIRSTNAME, LASTNAME = A_LASTNAME, DOB = A_DOB, ADDR1 = A_ADDR1, ADDR2 = A_ADDR2, CITY = A_CITY, ST = A_ST, ZIP = A_ZIP, EMAIL = A_EMAIL, PHONENO = A_PHONENO WHERE SID = A_SID;
END $$

DROP PROCEDURE IF EXISTS `PQ_UPDATE_TEACHER`$$
CREATE PROCEDURE `PQ_UPDATE_TEACHER` (
	A_TID			INT,
	A_USERNAME		VARCHAR(30),
	A_PASSWORD		VARCHAR(30), 
	A_FIRSTNAME		VARCHAR(30),
...
BEGIN
    UPDATE TEACHERS SET USERNAME = A_USERNAME, PASSWORD = A_PASSWORD, FIRSTNAME = A_FIRSTNAME, LASTNAME = A_LASTNAME, DEPARTMENT = A_DEPARTMENT, COLLEGE = A_COLLEGE, SUBJECTS = A_SUBJECTS, PHONENO = A_PHONENO, WEBSITE = A_WEBSITE WHERE TID = A_TID;
END $$
DELIMITER ;

DELIMITER $$
DROP PROCEDURE IF EXISTS `PQ_LOG_IN_STUDENT`$$
CREATE PROCEDURE `PQ_LOG_IN_STUDENT` (
	A_USERNAME	VARCHAR(30),
    A_PASSWORD	VARCHAR(30))
BEGIN
...
DROP PROCEDURE IF EXISTS `PQ_GET_STUDENT`$$
CREATE PROCEDURE `PQ_GET_STUDENT` (
	A_SID	INT)
BEGIN
    SELECT * FROM STUDENTS WHERE SID = A_SID;
END $$
...
DROP PROCEDURE IF EXISTS `PQ_INSERT_STUDENT`$$
CREATE PROCEDURE `PQ_INSERT_STUDENT` (
	A_SID			INT,
	A_USERNAME		VARCHAR(30),
	A_PASSWORD		VARCHAR(30), 
	A_FIRSTNAME		VARCHAR(30),
...
CREATE PROCEDURE `PQ_GET_COURSES_IN_CAT` (
    ...
    A_SID		INT)
BEGIN
	SELECT * FROM COURSES WHERE CID NOT IN (SELECT COURSEHIST.CID FROM COURSEHIST INNER JOIN STUDENTS ON COURSEHIST.SID = STUDENTS.SID WHERE STUDENTS.SID = A_SID) AND CATEGORY = A_CATEGORY ORDER BY CLASSNAME;
END $$
...
DROP PROCEDURE IF EXISTS `PQ_SELECT_COURSES_BY_SID`$$
CREATE PROCEDURE `PQ_SELECT_COURSES_BY_SID` (
	A_SID		INT)
BEGIN
	SELECT * FROM COURSES INNER JOIN COURSEHIST ON COURSEHIST.CID = COURSES.CID WHERE COURSEHIST.SID = A_SID;
END $$
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `PQ_LOG_IN_TEACHER`$$
CREATE PROCEDURE `PQ_LOG_IN_TEACHER` (
	A_USERNAME	VARCHAR(30),
    A_PASSWORD	VARCHAR(30))
BEGIN
...
DROP PROCEDURE IF EXISTS `PQ_INSERT_TEACHER`$$
CREATE PROCEDURE `PQ_INSERT_TEACHER` (
	A_SID			INT,
	A_USERNAME		VARCHAR(30),
	A_PASSWORD		VARCHAR(30), 
	A_FIRSTNAME		VARCHAR(30),
	...
	A_WEBSITE		VARCHAR(30))
BEGIN
    INSERT INTO TEACHERS VALUES(A_SID, A_USERNAME, A_PASSWORD, A_FIRSTNAME, A_LASTNAME, A_EMAIL, A_DEPARTMENT, A_COLLEGE, A_SUBJECTS, A_PHONENO, A_WEBSITE);
END $$
...
DROP PROCEDURE IF EXISTS `PQ_JOIN_LECTURE`$$
CREATE PROCEDURE `PQ_JOIN_LECTURE` (
	A_CID		INT,
	A_TID		INT)
BEGIN
...
DROP PROCEDURE IF EXISTS `PQ_DELETE_COURSES_BY_CID`$$
CREATE PROCEDURE `PQ_DELETE_COURSES_BY_CID` (
	A_CID		INT)
BEGIN
...
DROP PROCEDURE IF EXISTS `PQ_MAKE_REPORTID`$$
CREATE PROCEDURE `PQ_MAKE_REPORTID` ()
BEGIN
